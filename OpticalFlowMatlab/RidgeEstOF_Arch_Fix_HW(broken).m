...
for i=1:sizeVy
    ...
    for j=1:sizeVx
        ...
        den = gxx*gyy-gxy^2;
        if (den<0.5)%32)%
            k1 = gtt-2*gxt*vx_l+gxx*vx_l*vx_l;
            k2 = -2*gyt*vy_l+2*gxy*vx_l*vy_l+gyy*vy_l*vy_l;
            k = (p/(n-p))*(k1+k2)/ratio;
        else
            k = 0;
        end;
...
if( john_test == 1)
    ...
    vx_sm = Saturate1(vx_sm(9:244,9:308)/2,7,-8);
    vy_sm = Saturate1(vy_sm(9:244,9:308)/2,7,-8);
