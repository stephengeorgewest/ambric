...
for i=1:sizeVy
    ...
    %gxx = double(GxGx_s.number(i,1)) ;
    %gyy = double(GyGy_s.number(i,1)) ;
    %gxy = double(GxGy_s.number(i,1)) ;
    %gxt = double(GxGt_s.number(i,1)) ;
    %gyt = double(GyGt_s.number(i,1)) ;
    %[vx_l, vy_l] = VelocityCalc_fix(gxx, gyy, gxy, gxt, gyt, fraction_bits);
    %[vx_l_true, vy_l_true] = VelocityCalc(gxx, gyy, gxy, gxt, gyt);
    %vx_scale = vx_l_true/vx_l
    %vy_scale = vy_l_true/vy_l
   %Vx(i, 1) = vx_l;
    %Vy(i, 1) = vy_l;
    %C(i, 1) = 1;
    %Kr(i, 1) = 0;
    ...
    for j=1:sizeVx
        ...
        %    a = 1;
        % end;
        gxx = double(GxGx_s.number(i,j)) ;
        gyy = double(GyGy_s.number(i,j)) ;
        gxy = double(GxGy_s.number(i,j)) ;
        gxt = double(GxGt_s.number(i,j)) ;
        gyt = double(GyGt_s.number(i,j)) ;
        gtt = double(GtGt_s.number(i,j)) ;
        [vx_l, vy_l] = VelocityCalc_fix(gxx, gyy, gxy, gxt, gyt, fraction_bits);
        [vx_true, vy_l_true] = VelocityCalc(gxx, gyy, gxy, gxt, gyt);
        %
        %saturate?
        
        if (vx_l>(8*(2^fraction_bits)))
            vx_l = 8*(2^fraction_bits);
        elseif (vx_l<-(8*(2^fraction_bits)))
            vx_l = -(8*(2^fraction_bits));
        end;
        if (vy_l>(8*(2^fraction_bits))) 
            vy_l = (8*(2^fraction_bits));
        elseif (vy_l<-(8*(2^fraction_bits)))
            vy_l = -(8*(2^fraction_bits));
        end;
        ...
        if (gxx*gyy-gxy^2<4)
            k1a_t = gtt;
            k1b_t = -gxt*vx_l_true;
            k1c_t = +gxx*vx_l_true*vx_l_true;
            k1_t = k1a_t+k1b_t+k1b_t+k1c_t;
            
            k2a_t = -gyt*vy_l_true;
            k2b_t = +gxy*vx_l_true*vy_l_true;
            k2c_t = +gyy*vy_l_true*vy_l_true;
            k2_t = k2a_t+k2a_t+k2b_t+k2b_t+k2c_t;
            
            k_full_t = k1_t + k2_t;
            ktrue =(p/(n-p))*k_full_t/ratio;
            
            k1a = gtt*(2^fraction_bits);
            k1b =  -(  (gxt*(2^fraction_bits)  )*vx_l);
            k1c1 = gxx*(2^fraction_bits);
            k1c2 = round((vx_l*vx_l)/(2^fraction_bits));
            k1c = +(  k1c1  )*(  k1c2  );
            k1 = k1a + round((k1b+ k1b +k1c)/(2^fraction_bits));
            
            k2a = -(gyt*(2^fraction_bits))*vy_l;
            k2b1 = gxy*(2^fraction_bits);
            k2b2 = round((vx_l*vy_l)/(2^fraction_bits));
            k2b = (k2b1)*(k2b2);
            k2c1 = gyy*(2^fraction_bits);
            k2c2 = round((vy_l*vy_l)/(2^fraction_bits));
            k2c = (k2c1)*(k2c2);
            k2 = (k2a + k2a) + round((k2b + k2b) +k2c)/(2^fraction_bits);
            
            k_add = k1+k2;
            k_full = round((k_add)/(2^fraction_bits));
            
            k = round((p/(n-p))*k_full/ratio);
        else
            k = 0;
        end;
%        if(j==1)
...
%         end;
        ...
        [vx_l, vy_l] = VelocityCalc_fix(gxx_n, gyy_n, gxy, gxt, gyt, fraction_bits);
        [vx_l_true, vy_l_true] = VelocityCalc(gxx, gyy, gxy, gxt, gyt);
        %vx_scale = vx_l_true/vx_l
        %vy_scale = vy_l_true/vy_l
        if (vx_l>(8*(2^fraction_bits)))
...
%         end;
        ...
        Vx_full(i,j) = vx_l_true;
        Vx(i, j) = vx_l/(2^fraction_bits);
        Vy_full(i,j) = vy_l_true;
        Vy(i, j) = vy_l/(2^fraction_bits);
    end;
end;
...
if( john_test == 1)
    ...
    smooth_v_mask_gain=64;
    vx_sm1 = vx_sm(4*236+1:5*236,:)/(2^fraction_bits)/smooth_v_mask_gain;
    vy_sm1 = vy_sm(4*236+1:5*236,:)/(2^fraction_bits)/smooth_v_mask_gain;
    view_tab_del(vx_sm);
...
if(divisor ==0)
    ...
    vx = vx_dividend/divisor;
    vy = vy_dividend/divisor;
end;
...
%The function for calculating velocity
function [vx, vy] = VelocityCalc_fix(gxx, gyy, gxy, gxt, gyt, fraction_bits)
vx_dividend = gxt*gyy-gxy*gyt;
vy_dividend = gxx*gyt-gxy*gxt;
if (vx_dividend==0 && vy_dividend==0)
    a = 1;
...
if(divisor ==0)
    ...
    vx = round(vx_dividend/divisor*(2^fraction_bits));
    vy = round(vy_dividend/divisor*(2^fraction_bits));
%    vx = Vx_full;%fix((Vx_full));
%    vy = Vx_full;%fix((Vy_full));
end;
