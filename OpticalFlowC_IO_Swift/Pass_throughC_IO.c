	...
	if(h!=0)
	{
		...
		if(reset_val==0)
		{
			...
			if(config_val==0)
			{
				...
				{
					int Pass_Through[data_size];
					for(i=0; i<data_size;i++)
					{
						Pass_Through[i]=0;
					}
					int read_handle_x = swift_read_async(h, Pass_Through, data_size-1, 0, 0);
					if(read_handle_x==0)
						puts("broken read x");
					...
					*/
					...
					while(read_x_done == 0)
					{
						if(read_x_done==1&&num_x_read!=-1)
						{
							...
							printf("Pass_Through done %d",num_x_read);
						}
						else
					...
					for(i=1; i<data_size; i++)
					{
						if(Pass_Through[i-1]!=data[i] )
							printf("!error! data[%02d]=%03d\tpass_through=%03d\n",i,data[i], Pass_Through[i-1]);
					}
