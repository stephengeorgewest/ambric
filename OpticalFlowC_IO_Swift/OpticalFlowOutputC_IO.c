...
int main(void) {
	...
	if(h!=0)
	{
		...
		if(reset_val==0)
		{
			...
			int config_val = config_dev(h, "OpticalFlowOutputOnly.cfg");
			if(config_val==0)
			{
				...
				int read_handle_x = swift_read_async(h, Vx, 10, 0, SWIFT_CHANNEL0);
				if(read_handle_x==0)
					puts("broken read x");
				int read_handle_y = swift_read_async(h, Vy, 10, 0, SWIFT_CHANNEL1);
				if(read_handle_y==0)
					puts("broken read y");
				...
				while(num_x_read==-1 || num_y_read==-1 || read_x_done == 0 || read_y_done ==0)
				{
					...
					{
						int data[] ={0};
						int write_handle = swift_write_async(h,data,1,0,SWIFT_CHANNEL0);
						if(!(write_handle==0 | write_handle==-1))
						{
							int num_write=swift_wait_async(h, write_handle);
							if(num_write==-1)
								puts(swift_get_last_error_string());
							else
								printf("\nsome data written = %d\n",num_write);
					...
					printf(".");
					Sleep(100);
					time++;//every 100th of second
					if(time>1*60*10)//stop after 1 min
					{
