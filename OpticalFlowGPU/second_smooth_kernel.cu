...
convolve2_x_t(float *d_Result, float *d_Data, int width, int height, int pitch) {
	...
	__shared__ float data[XCONV2_TILE_WIDTH + 2*FILTER4_RAD];
...
convolve2_y(float *d_Result, float *d_Data, int image_width, int image_height, int pitch) {
	...
	__shared__ float data[YCONV2_TILE_WIDTH*(2*FILTER4_RAD+YCONV2_TILE_HEIGHT)];
...
__global__ void 
convolve_xy(float *d_Result, float *d_Data, int image_width, int image_height) {
...
__global__ void 
convolve_xy_t(float *d_Result, float *d_Data, int image_width, int image_height, int pitch) {

	__shared__ float data[(XYCONV_TILE_WIDTH+2*FILTER4_RAD)*(2*FILTER4_RAD+XYCONV_TILE_HEIGHT)];

	// X direction--------------------------------------------------------------------
	//Current tile and apron limits, relative to row start
	const int         tileStartX = IMUL(blockIdx.x, XYCONV_TILE_WIDTH);
	const int           tileEndX = tileStartX + XYCONV_TILE_WIDTH - 1;
	const int        apronStartX = tileStartX - FILTER4_RAD;
	const int          apronEndX = tileEndX   + FILTER4_RAD;

	//Clamp tile and apron limits by image borders
	const int    tileEndClampedX = min(tileEndX, image_width - 1);
	const int apronStartClampedX = max(apronStartX, 0);
	const int   apronEndClampedX = min(apronEndX, image_width - 1);

	//Row start index in d_Data[]
	const int          rowStart = IMUL(blockIdx.y, pitch);
	const int apronStartAligned = tileStartX - FILTER_RAD_ALIGNED;
	const int           loadPos = apronStartAligned + threadIdx.x;
	//const int 	   writePos = tileStartX + threadIdx.x;


	// Y direction--------------------------------------------------------------------
	//Current tile and apron limits, in rows
	const int         tileStartY = IMUL(blockIdx.y, XYCONV_TILE_HEIGHT);
	const int           tileEndY = tileStartY + XYCONV_TILE_HEIGHT - 1;
	const int        apronStartY = tileStartY - FILTER4_RAD;
	const int          apronEndY = tileEndY   + FILTER4_RAD;

	//Clamp tile and apron limits by image borders
	const int apronStartClampedY = max(apronStartY, 0);
	const int   apronEndClampedY = min(apronEndY, image_height - 1);

	//Current column index
	const int       columnStart = IMUL(blockIdx.x, XYCONV_TILE_WIDTH) + threadIdx.x; //same as writePos




	if(columnStart < image_width && loadPos >= apronStartX) {
		//Shared and global memory indices for current column
		int smemPos = IMUL(threadIdx.y, XYCONV_TILE_WIDTH+2*FILTER4_RAD) + (loadPos-apronStartX);
		int gmemPos = IMUL(apronStartY + threadIdx.y, pitch) + columnStart;

		//Load global memory values, if indices are within the image borders,
		//or initialize with zero otherwise
		data[smemPos] = ((apronStartY + threadIdx.y >= apronStartClampedY) && (apronStartY + threadIdx.y <= apronEndClampedY) 
				&& (loadPos >= apronStartClampedX) && (loadPos <= apronEndClampedX)) 
				? d_Data[gmemPos] : 0;
	}
	
	__syncthreads();

	if(columnStart < image_width && threadIdx.y < XYCONV_TILE_HEIGHT && tileStartY + threadIdx.y < image_height) {
		//Shared and global memory indices for current column
		//int smemPos = IMUL(threadIdx.y + FILTER4_RAD, 2*FILTER4_RAD + XYCONV_TILE_WIDTH) + threadIdx.x;
		int smemPos = IMUL(threadIdx.y + FILTER4_RAD, XYCONV_TILE_WIDTH) + threadIdx.x;
		int gmemPos = IMUL(tileStartY + threadIdx.y, pitch) + columnStart;

		float sum = 0;

		if(!( ( columnStart < FILTER4_RAD ) || ( columnStart >= (image_width-FILTER4_RAD) ) || ( (tileStartY + threadIdx.y) < FILTER4_RAD) || 			    ( (tileStartY + threadIdx.y) >= (image_height-FILTER4_RAD) )  ))
			sum = 25*data[smemPos-XYCONV_TILE_WIDTH-1] + 30*data[smemPos-XYCONV_TILE_WIDTH] + 25*data[smemPos-XYCONV_TILE_WIDTH+1]
			    + 30*data[smemPos-1] + 36*data[smemPos] + 30*data[smemPos+1]
			    + 25*data[smemPos+XYCONV_TILE_WIDTH-1] + 30*data[smemPos+XYCONV_TILE_WIDTH] + 25*data[smemPos+XYCONV_TILE_WIDTH+1];
