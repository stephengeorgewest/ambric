...
void second_order_smoothing() 
{
	...
	for( int i=0; i<6; i++ ) {
		/*if( width<=512 && width%16 == 0 )
			convolve2_x<<<height,width,width_in_bytes>>>(convolved2_x[i], smoothed_2nd[i], width, height, pitch3); //faster if width<512
		else
			convolve2_x_t<<<xgrid3,xblock3>>>(convolved2_x[i], smoothed_2nd[i], width, height, pitch3); //tiled version
		CUT_CHECK_ERROR("Kernel Execution Failed");
		
		convolve2_y<<<ygrid3,yblock3>>>(convolved2_y[i], convolved2_x[i], width, height, pitch3);   //tiled version - faster for y
		CUT_CHECK_ERROR("Kernel Execution Failed");
		*/
		//convolve_xy<<<height,320,width*sizeof(float)*3>>>(convolved2_y[i], smoothed_2nd[i], width, height);  //works best for 320x widths
		convolve_xy_t<<<xygrid,xyblock>>>(convolved2_y[i], smoothed_2nd[i], width, height, pitch3);	//tiled version - doesn't work
