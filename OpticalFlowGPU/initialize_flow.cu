...
void init_flow() {
	...
	for( int i=0; i<3; i++ ) {
		...
		CUDA_SAFE_CALL( cudaMemset2D( (void**) &deriv_x[i], d_pitch, 0, width_in_bytes, height ) );
		CUDA_SAFE_CALL( cudaMemset2D( (void**) &deriv_y[i], d_pitch, 0, width_in_bytes, height ) );
		CUDA_SAFE_CALL( cudaMemset2D( (void**) &deriv_t[i], d_pitch, 0, width_in_bytes, height ) );
	...
	for( int i=0; i<3; i++ ) {
		...
		CUDA_SAFE_CALL( cudaMemset2D( (void**) &smoothed[i], d_pitch, 0, width_in_bytes, height ) );
		CUDA_SAFE_CALL( cudaMemset2D( (void**) &convolved_x[i], d_pitch, 0, width_in_bytes, height ) );
		CUDA_SAFE_CALL( cudaMemset2D( (void**) &convolved_y[i], d_pitch, 0, width_in_bytes, height ) );
	}
	...
	for( int i=0; i<6; i++ ) {
		...
		CUDA_SAFE_CALL( cudaMemset2D( (void**) &smoothed_2nd[i], d_pitch, 0, width_in_bytes, height ) );
		CUDA_SAFE_CALL( cudaMemset2D( (void**) &convolved2_x[i], d_pitch, 0, width_in_bytes, height ) );
		CUDA_SAFE_CALL( cudaMemset2D( (void**) &convolved2_y[i], d_pitch, 0, width_in_bytes, height ) );
	}
	...
	printf("xygrid = (%d,%d), xyblock = (%d,%d)\n", (width+XYCONV_TILE_WIDTH-1)/XYCONV_TILE_WIDTH, (height+XYCONV_TILE_HEIGHT-1)/XYCONV_TILE_HEIGHT, FILTER_RAD_ALIGNED+XYCONV_TILE_WIDTH+FILTER4_RAD, XYCONV_TILE_HEIGHT + 2*FILTER4_RAD );
	...
	CUDA_SAFE_CALL( cudaMemset2D( (void**) &Vx, d_pitch, 0, width_in_bytes, height ) );
	CUDA_SAFE_CALL( cudaMemset2D( (void**) &Vy, d_pitch, 0, width_in_bytes, height ) );
	...
	CUDA_SAFE_CALL( cudaMemset2D( (void**) &Vx_x, d_pitch, 0, width_in_bytes, height ) );
	CUDA_SAFE_CALL( cudaMemset2D( (void**) &Vy_x, d_pitch, 0, width_in_bytes, height ) );
	CUDA_SAFE_CALL( cudaMemset2D( (void**) &Vx_y, d_pitch, 0, width_in_bytes, height ) );
	CUDA_SAFE_CALL( cudaMemset2D( (void**) &Vy_y, d_pitch, 0, width_in_bytes, height ) );
